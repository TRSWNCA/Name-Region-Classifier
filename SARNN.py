    ...
    for i in range(EPOCH):
        ...
        if len(gru_loss) > 0:
            if gru_loss[-1] < sum_loss:
                val = math.exp(-(sum_loss - gru_loss[-1]) / 40000)
                print(val)
                if random.randint(0, 99) > val * 100:
...
grn_loss, gru_val = all(train_X, train_Y, valid_X, valid_Y)
...
plt.figure(0)
plt.plot(x, gru_val, label='GRU-SA')
plt.plot(x, rnn_val, label='GRU')
plt.title('Accuracy')
plt.legend(loc='upper left')
plt.savefig('Compare Accuracy.png')

plt.figure(1)
plt.plot(x, gru_loss, label='GRU-SA')
plt.plot(x, rnn_loss, label='GRU')
plt.title('Loss')
plt.legend(loc='upper left')
